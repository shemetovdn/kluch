    ...
    class LangRequest extends Request
{
    ...
    public function getLangUrl()
    {
        if ($this->_lang_url === null) {
            ...
            Yii::$app->lang->setCurrent($lang_url);

            $tmpLang=Yii::$app->lang->languagesUrls[Yii::$app->lang->getCurrent()];

            if( $lang_url !== null && $lang_url === $tmpLang && $tmpLang){
                if(strpos($this->_lang_url, $tmpLang) === 1 ) {
                    $this->_lang_url = substr($this->_lang_url, strlen($tmpLang) + 1);
                }
            }
        }
