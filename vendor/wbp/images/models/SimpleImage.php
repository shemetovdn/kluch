...
class SimpleImage {
    ...
    function crop($x1, $y1, $x2, $y2) {
        ...
        // Perform crop
        $new = imagecreatetruecolor($x2 - $x1, $y2 - $y1);
        imagealphablending($new, false);
        imagesavealpha($new, true);
        $transparent = imagecolorallocatealpha($new, 255, 255, 255, 127);
        imagefilledrectangle($new, 0, 0, $x2 - $x1, $y2 - $y1, $transparent);
        ...
        $dst_x=0;
        $dst_y=0;
        if($x1<0){$dst_x=abs($x1); $x1=0;}
        if($y1<0){$dst_y=abs($y1); $y1=0;}

        imagecopyresampled($new, $this->image, $dst_x, $dst_y, $x1, $y1, $crop_width, $crop_height, $crop_width, $crop_height);
