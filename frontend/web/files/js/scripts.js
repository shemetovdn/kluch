...
$(document).ready(function(){
    ...
    function openDrop (that, backGr, fontColor, divList) {
        ...
        // var liCount = 0;
        var liCount = that.parents('.drop-unit-container').find(divList).find('div').length - 2;
        var totalLiHeight = 30 * liCount;
        console.log(liCount);

        var height = 80 + totalLiHeight;
        if ($(window).width() < 768) {
            height = 50 + totalLiHeight;
        }
    ...
    // Filter Price input mask
    $('.li-price input').priceFormat();
});
